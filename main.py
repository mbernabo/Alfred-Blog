...
def register():
    ...
    if form.validate_on_submit():
        ...
        if db.session.execute(db.select(User).filter_by(email=email)).scalar():
            flash('El e-mail ya existe en la base de datos. Será redirigido a la Login Page', category='success')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and check_password_hash(user.password, form.password.data):
            ...
            flash('Your credentials are wrong. Please try again.', category='danger')
            return redirect(url_for('login'))
    return render_template("login.html", form=form)


# with app.app_context():
#     print(db.session.execute(db.select(User).filter_by(email='jj@jj.com')))
